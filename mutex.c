...
iou_mutex_enter(reactor_t * reactor, iou_mutex_t * mutex) {
    ...
    if (!iou_mutex_knock(reactor, mutex))
    while (0 != atomic_exchange_explicit(&mutex->value, -1, memory_order_acq_rel))
...
iou_mutex_taken(reactor_t * reactor, const iou_mutex_t * mutex) {
    return 0 != atomic_load_explicit(&mutex->value, memory_order_acquire);
}

void
iou_mutex_leave(reactor_t * reactor, iou_mutex_t * mutex) {
    assert(iou_mutex_taken(reactor, mutex));
    if (-1 == atomic_exchange_explicit(&mutex->value, 0, memory_order_acq_rel))
...
iou_mootex_knock(reactor_t * reactor, iou_mootex_t * mootex) {
    ...
    if (whoami == atomic_load_explicit(&mootex->owner, memory_order_acquire)) {
        ++mootex->depth;
        return true;
    }
    ...
    if (iou_mutex_knock(reactor, &mootex->mutex)) {
        ...
        mootex->depth = 1;
        assert(!atomic_load_explicit(&mootex->owner, memory_order_acquire));
        atomic_store_explicit(&mootex->owner, whoami, memory_order_release);
        return true;
    }
...
iou_mootex_leave(reactor_t * reactor, iou_mootex_t * mootex) {
    uintptr_t whoami = reactor_current(reactor);
    if (UNLIKELY(whoami != atomic_load_explicit(&mootex->owner, memory_order_acquire)))
        abort();
    if (!--mootex->depth) {
        atomic_store_explicit(&mootex->owner, 0, memory_order_release);
